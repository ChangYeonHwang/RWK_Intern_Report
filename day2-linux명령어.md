...
# ls
# mv_test.txt
//practice 폴더로 이동, 리스트 명령어를 통해 폴더에 mv_test.txt 파일이 복사 되었음을 확인
```
...
# 5번 index를 갖는 배열의 원소 : 1

# arr[5]="not five"
# echo "5번 index를 갖는 배열의 원소 : ${arr[5]}"
# 5번 index를 갖는 배열의 원소 : not five
...
+ AND은 -a, OR은 -o를 이용해서 조건을 추가 할 수 있다
...
+ 명령어의 종료 코드는 "$?"로 가장 최근 실행한 명령어의 종료 코드가 있다
